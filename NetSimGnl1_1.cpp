    ...
    if((Evts[i].TimEv*1e3f>=EvTime) && (Evts[i].TimEv*1e3f<(EvTime+dt)))
    {
      ...
      else if(Evts[i].Type==EVTCUNTINJ)
      {
        for(unsigned int m=NIdx[Evts[i].Pp];m<(NIdx[Evts[i].Pp]+ConfSta[Evts[i].Pp].NeuNum);m++)
        {
          ...
          BgSt[m].std=Evts[i].Var2*(1.0f+(float)log(-0.1/dt));//normalize to dt=0.1ms
...
void NetSimGnl1_1::RunInit(int thNum,int sop)
{
...
void NetSimGnl1_1::SimThd(unsigned int thid)
{
...
void NetSimGnl1_1::SimOne(void)
{
  for(unsigned int i=0; (i < Endtime) || NoLmt ; i++)  //calculation
  {
    ...
    if((i % ReportStep)==0)
    {
      ...
      {
        cout
        <<"Experiment time="<<EvTime*1e-3f<<"s "
        <<"real time="<<(chrono::duration_cast<chrono::nanoseconds>(t2-t1).count())*1e-9<<"s "
        <<":infinity End time             \r"
        <<flush;
      }
...
//build up noise
  ...
  else
  {
    ...
    { th[i] = thread(&NetSimGnl1_1::SimThd,this,i); }
